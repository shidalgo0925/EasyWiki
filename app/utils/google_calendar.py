...
def create_event(
    ...
    Retorna el evento creado con su 'id'.
    """
    service = build("calendar", "v3", credentials=creds)
    tz = ZoneInfo(LOCAL_TZ)
    ...
    body = {
        ...
        "start": {"dateTime": start_dt.astimezone(tz).isoformat(), "timeZone": LOCAL_TZ},
        "end": {"dateTime": end_dt.astimezone(tz).isoformat(), "timeZone": LOCAL_TZ},
    }
...
def sync_focus_items_to_calendar(creds, focus_items, fecha: date) -> List[Dict[str, Any]]:
    """
    Crea eventos de Google Calendar a partir de una lista de FocusItem.
    Solo sincroniza items que no tengan google_event_id y que tengan hora_inicio.
    Retorna lista de eventos creados.
    """
    created = []
    tz = ZoneInfo(LOCAL_TZ)

    for item in focus_items:
        if item.google_event_id or not item.hora_inicio:
            continue

        start_dt = datetime.combine(fecha, item.hora_inicio).replace(tzinfo=tz)
        end_dt = start_dt + timedelta(minutes=item.duracion_min or 30)
        ...
        try:
            event = create_event(
                ...
                start_dt=start_dt,
                end_dt=end_dt,
                description=f"Categoría: {item.categoria or 'N/A'} | Prioridad: {item.prioridad}",
            )
            item.google_event_id = event.get("id")
