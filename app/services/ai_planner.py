...
class AIPlanner:
    ...
    def suggest(self, goals_raw: str, horizonte: str, restricciones: dict, agenda: list, user_id: int = 1):
        """
        ...
        """
        # Si el proveedor soporta contexto completo (OpenAI), usar suggest con user_id
        if hasattr(self.provider, 'suggest') and 'user_id' in self.provider.suggest.__code__.co_varnames:
            return self.provider.suggest(goals_raw, user_id)
        # Fallback a método clásico (LocalAdapter)
